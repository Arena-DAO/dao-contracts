...
use cw_orch::prelude::*;
use cw_orch_osmosis_test_tube::osmosis_test_tube::{Account, SigningAccount};
use cw_orch_osmosis_test_tube::OsmosisTestTube;
use dao_cw_orch::{
    DaoDaoCore, DaoExternalCwAbc, DaoExternalTokenfactoryIssuer, DaoProposalSudo,
...
fn setup() -> Result<(
    OsmosisTestTube,
    DaoExternalCwAbc<OsmosisTestTube>,
    u64,
    TestAccounts,
)> {
    ...
    // Upload ABC contract
    let abc = DaoExternalCwAbc::new("cw_abc", chain.clone());
    abc.upload()?;

    // Upload cw-tokenfactory-issuer
    let token_factory_issuer =
        DaoExternalTokenfactoryIssuer::new("cw_tokenfactory_issuer", chain.clone());
